...
import math as mt
from scipy.interpolate import interp2d, pchip_interpolate
import _pickle as cPickle
import cfsites_forcing_reader as cfreader
...
cdef extern:
    ...
    # the following function is for cloud overlap
...
cdef class RadiationNone(RadiationBase):
    def __init__(self):
        ...
        return
    cpdef initialize(self, ReferenceState Ref, Grid Gr, GridMeanVariables GMV, TimeStepping TS):
        RadiationBase.initialize(self, Ref, Gr, GMV, TS)
        return
    cpdef update(self, ReferenceState Ref, Grid Gr, GridMeanVariables GMV, TimeStepping TS):
        return
    cpdef initialize_io(self, NetCDFIO_Stats Stats):
...
cdef class RadiationRRTM(RadiationBase):
    ...
    cpdef initialize(self, ReferenceState Ref, Grid Gr, GridMeanVariables GMV, TimeStepping TS):
        ...
        RadiationBase.initialize(self, Ref, Gr, GMV, TS)
        self.heating_rate = np.zeros((Gr.nz,), dtype=np.double)
        self.net_lw_flux = np.zeros((Gr.nz,), dtype=np.double)    
        ...
        dp = np.abs(Ref.p0[-1] - Ref.p0[-2])
        self.patch_pressure = np.minimum(self.patch_pressure, Ref.p0[-1] - dp)
        n_profile = 0
        ...
        if self.n_buffer > 0:
            ...
            xi = np.array([self.p_ext[self.n_buffer+1],self.p_ext[self.n_buffer],Ref.p0[-1],Ref.p0[-2] ],dtype=np.double)
            print(xi)
            # interpolation for temperature
            ti = np.array([self.t_ext[self.n_buffer+1],self.t_ext[self.n_buffer], GMV.T.values[-1], GMV.T.values[-2] ], dtype = np.double)
            ...
            qv = GMV.QT.values[nz-2] - GMV.QL.values[nz-2]
            rv_m2 = qv[nz-2]/(1.0 - qv[nz-2])
            qv = GMV.QT.values[nz-1] - GMV.QL.values[nz-1]
            rv_m1 = qv[nz-1]/(1.0 - qv[nz-1])
        ...
        self.p_full[0:nz] = Ref.p0 #Ref.p0_half_global[gw:nz+gw] # at cell center
        ...
        return
                ...
                #TODO call RRTMG with Scampy profile
        #TODO populate self.dTdt from RRTMG
#         update 
    ...
    cpdef update(self, ReferenceState Ref, Grid Gr, GridMeanVariables GMV, TimeStepping TS):
        ...
        tsfc_in = np.ones((n_pencils),dtype=np.double,order='F') * (GMV.T.values[Gr.gw] + GMV.T.values[Gr.gw-1])/2.0
        # with nogil:
        ...
        for k in xrange(nz):
            tlay_in[k] = GMV.T.values[k]
            qv = GMV.QT.values[k] - GMV.QL.values[k]
            h2ovmr_in[k] = qv/ (1.0 - qv)* Rv/Rd * self.h2o_factor
            rl_full[k] = (GMV.QL.values[k])/ (1.0 - qv)
            qi = 0
            # once qi is part of the model, replace with GMV.QI
            ri_full[k] = (qi)/ (1.0 - qv)
            cliqwp_raw[k] = ((GMV.QL.values[k])/ (1.0 - qv)
                               *1.0e3*(self.pi_full[k] - self.pi_full[k+1])/g)
            cicewp_raw[k] = ((qi)/ (1.0 - qv)
                               *1.0e3*(self.pi_full[k] - self.pi_full[k+1])/g)
            cldfr_raw[k] = GMV.cloud_fraction.values[k]
        with nogil:
            for k in xrange(nz_full):
                ...
                if self.uniform_reliq:
                    reliq_raw[k] = 14.0*GMV.cloud_fraction.values[k]
                else:
                    reliq_raw[k] = ((3.0*self.p_full[k]/Rd/tlay_in[k]*rl_full[k]/
                                        fmax(GMV.cloud_fraction.values[k],1.0e-6))/(4.0*pi*1.0e3*100.0))**(1.0/3.0)
...
#                 tlev_in[0] = tsfc_in
        ...
        # dTdt = profile at some time
        for k in xrange(Gr.gw, Gr.nzg-Gr.gw):
            
#             self.net_lw_flux[k] = uflx_lw_out[k] - dflx_lw_out[k]

             #TODO: ask about sec->day conversion, double-check this eq.
            self.dTdt[k] =((hr_lw_out[k] + hr_sw_out[k])/86400.0)
            
            qv = GMV.QT.values[k] - GMV.QL.values[k]
            GMV.H.radiation[k] = self.convert_forcing_prog_fp(Ref.p0_half[k],GMV.QT.values[k], qv, GMV.T.values[k], qv, self.dTdt[k])
            GMV.H.tendencies[k] += GMV.H.radiation[k]
        return
